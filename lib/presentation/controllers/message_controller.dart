  ...
  Future<void> sendMessage(String plaintext) async {
    ...
    try {
      ...
      if (sessionId == null || sessionId.isEmpty) {
        await popup(
          AppStrings.messageNoSessionTitle,
          AppStrings.messageNoSessionMessage,
        );
        return;
      }
      ...
      if (plaintext.isEmpty) {
        await popup(
          AppStrings.messageNoDataTitle,
          AppStrings.messageNoDataMessage,
        );
        return;
      }
      ...
      if (aesKey == null || aesKey.isEmpty) {
        await popup(
          AppStrings.messageNoAesKeyTitle,
          AppStrings.messageNoAesKeyMessage,
        );
        return;
      }

      final EncryptionResult encryptionResult = await encryptionService.encrypt(
        plaintext,
        aesKey,
      );
      ...
      final response = await messageService.sendMessage(
        SendMessageDto(
          ...
          algorithm:
              'AES', // Hardcode, to jest okej ponieważ nie ma wyboru algorytmu na ten moment, może będzie kiedyś więc zostawiam
        ),
      );
      ...
      if (response.body == null) {
        await popup(
          AppStrings.messageUnexpectedErrorTitle,
          AppStrings.sendMessageFailed,
        );
        return;
      }
